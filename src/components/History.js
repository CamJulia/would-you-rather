        ...
        <div className="flexContainer">
          <div className="flexItem">
            ...
            {askedQuestions.map(question => {
              return (
                <div key={question.id + 'myQuestion'}>
                  <p>
          ...
          <div className="flexItem">
            ...
            {answeredQuestions.map(question => {
              ...
              const percentOne =
                (100 / (votesOptionOne + votesOptionTwo)) * votesOptionOne;
              const percentTwo =
                (100 / (votesOptionOne + votesOptionTwo)) * votesOptionTwo;

              if (question[1] === 'optionOne') {
                return (
                  <div key={question.id + question[0].optionOne.text}>
                    <p>
                      ...<b>{question[0].optionOne.text}</b> than{' '}
                      {question[0].optionTwo.text}!
                    </p>
                    <p>
                      (Just like <b>{percentOne}%</b> of players.)
                    </p>
                    <hr />
                  </div>
                );
                ...
                return (
                  <div key={(question.id = question[0].optionTwo.text)}>
                    <p>
                      ...<b>{question[0].optionTwo.text}</b> than{' '}
                      {question[0].optionOne.text}!
                    </p>
                    <p>
                      (Just like <b>{percentTwo}%</b> of players.)
                    </p>
                    <hr />
                  </div>
                );
